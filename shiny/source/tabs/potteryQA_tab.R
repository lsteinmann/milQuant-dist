potteryQA_tab <- tabItem(
  ...
  fluidRow(
    infoBox(title = "Info", value = textOutput("potteryQA_overview"),
            icon = icon("list"),
  ...
  fluidRow(
    box(
      width = 3, height = 650,
      textInput(inputId = "potteryQAPlot_1_title", label = "Title",
                value = "", placeholder = "Enter title here"),
      textInput(inputId = "potteryQAPlot_1_subtitle", label = "Subtitle",
                placeholder = "Enter subtitle here"),
      uiLayerSelector("QA_layers"),
      prettyRadioButtons(inputId = "potteryQAPlot_1_display",
                   ...
                   choices = list("Do not display Context" = "none",
                                  ...
                                  "Context on x-Axis" = "x")),
      prettyRadioButtons(inputId = "potteryQAPlot_1_bars",
                   label = "Position of Bars", icon = icon("check"),
                   inline = TRUE, animation = "jelly",
                   choices = list("Dodged Bars" = "dodge2",
                                  "Stacked Bars" = "stack")),
      downloadPlotButtons("potteryQAPlot_1_download")
    ),
    box(
      width = 9, height = 650,
      plotlyOutput("potteryQAPlot_1", height = 620) %>% mq_spinner()
    )
  )
)
